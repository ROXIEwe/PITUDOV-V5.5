...
if not Executed then
	game:GetService("StarterGui"):SetCore("SendNotification", {Title = "PITUDO:V V5.5", Text = "Que onda nub " .. game.Players.LocalPlayer.Name .. " ,SUICIDATE IMBECIL Y SERAS ALGUIEN", Duration = 10})
else
	game:GetService("StarterGui"):SetCore("SendNotification", {Title = "PITUDO:V V5.5", Text = "EL PITUDO:V ESTA CARGADO", Duration = 10})
...
task.spawn(function()
	...
	while true do
		game.CoreGui["PITUDO:V V5.5"].Main.TopBar.Title.Text = "PITUDO:V V5.5                                                  ".. os.date("%B %d, %Y | %X")
		task.wait()
	end
...
Pitudo:vHub:setTheme("Glow",Color3.fromRGB(0, 0, 255))

Pitudo:vHub:setTheme("Accent",Color3.fromRGB(255, 0, 0))
...
local Name = "PITUDO:V V5.5/config.json"
...
if not isfolder("PITUDO:V V5") then
	makefolder("PITUDO:V V5")
...
ActualSettings.AutoLaunchPitito:vCMDS = convert(Settings.AutoLaunchPititoCMDS)
...
local Main = Pitudo:vHub:addPage("Main", 5012544693)
...
MainGodModesSection:addButton("Universal GodMode", function() -- Universal God Mode Was Not Made By Roxiewe
...
MainSection4:addToggle("Loop Kill",nil,function(LoopKill)
	if LoopKill then
		...
		if not o then
			game:GetService("StarterGui"):SetCore("SendNotification",{Title = "PITUDO:V V5",Text = Target .." was not found on the player list",Duration = 2})
			return
		end

		if workspace.Live[Target].Humanoid.Sit then
			game:GetService("StarterGui"):SetCore("SendNotification",{Title = "PITUDO:V V5",Text = "Player is sitted down / player has antikill",Duration = 2})
			return
		end
		if LoopKilling then
			LoopKilling = false
		...
		while LoopKilling do
			...
			if not Target or not workspace.Live:FindFirstChild(Target) then
				sendNotification("PITUDO:V V5",tostring(Target) .. "was not found on the player list")
				LoopKilling = false
				break
			end
			if not workspace.Live[Target]:FindFirstChild("HumanoidRootPart") then
				sendNotification("PITUDO:V V5","Target does not have a humanoid root part",2)
				LoopKilling = false
				break
			end
			if workspace.Live[Target].Humanoid.Sit then
				sendNotification("PITUDO:V V5","Target is sitted down",2)
				LoopKilling = false
				break
			end
...
local Misc = Pitudo:vHub:addPage("Misc", 5012544693)
local MiscSection1 = Misc:addSection("Misc Toggles | Section 1")

MiscSection1:addSlider("Tele-Speed", 0, 0, 5000, function(telespeed) -- This TeleSpeed Was Not Made By Roxiewe,I just optimized a bit and changed some things
...
MiscSection1:addTextbox("Player Tracker", "", function(TrackValue, focusLost)
	...
	if focusLost then
		Pitudo:vHub:Notify("Tracking : ", TrackValue)
	end
end)
...
task.spawn(function()
	MiscSection1:addToggle("Player-Tracker",nil,function(Track)
		if PlayerValue == "" or not PlayerValue then
			game:GetService("StarterGui"):SetCore("SendNotification", {Title = "PITUDO:V V5", Text = "Type a player to track",Duration = 3})
			return
		end
		for _,v in pairs(game.Players:GetPlayers()) do
		...
		if not game.Players:FindFirstChild(PlayerValue) then
			game:GetService("StarterGui"):SetCore("SendNotification", {Title = "PITUDO:V V5", Text = PlayerValue .. " was not found on the player list",Duration = 3})
		end
		if Track then
...
task.spawn(function()
	MiscSection1:addToggle("Player Ki-Tracker",nil,function(KiTracker)
		...
		if not game.Players:FindFirstChild(PlayerToKiTrack) then
			game:GetService("StarterGui"):SetCore("SendNotification", {Title = "PITUDO:V V5", Text = PlayerToKiTrack .. " was not found on the player list",Duration = 3})
		end
...
task.spawn(function()
	MiscSection1:addToggle("GOD-Punch", nil, function(GOD)
		if GOD then
			if not Player.Backpack:FindFirstChild("Super Volley") and not Player.Character:FindFirstChild("Super Volley") then game:GetService("StarterGui"):SetCore("SendNotification", { Title = "PITUDO:V V5", Text = "Buy Super Volley", Duration = 5 }) return end
...
MiscSection2:addButton("Anti-Wipe",function()
	...
	sendNotification("PITUDO:V V5","Anti-Wipe Enabled",2)

	local get
	get = hookfunction(game.HttpGet, function(self, url, ...)
		if url:lower():find("blacklist") then
			sendNotification("PITUDO:V V5", "A request to a blacklist has been blocked", 2)
			return nil
		else
			return get(self, url, ...)
		end
	...
	Hook = hookmetamethod(game, "__namecall", function(...)
		...
		if self == senzuRemote and method:lower() == "fireserver" and not Args[2] then
			sendNotification("PITUDO:V V5", "SOMEONE TRIED TO WIPE YOUR ACCOUNT WITH EATSENZU REMOTE", 2)
			return nil
		elseif self == inputRemote and method:lower() == "fireserver" and (not Args[2] or type(Args[2]) ~= "table" or (type(Args[2]) == "table" and (Args[2][1] ~= "md" and Args[2][1] ~= "m2" and Args[2][1] ~= "e" and Args[2][1] ~= "blockoff" and Args[2][1] ~= "blockon" and Args[2][1] ~= "x" and Args[2][1] ~= "xoff")) or not Args[3] or typeof(Args[3]) ~= "CFrame") then
			sendNotification("PITUDO:V V5", "SOMEONE TRIED TO WIPE YOUR ACCOUNT WITH INPUT REMOTE", 2)
			return nil
		elseif self == chatRemote and method:lower() == "fireserver" and (not Args[2] or type(Args[2]) ~= "table") then
			sendNotification("PITUDO:V V5", "SOMEONE TRIED TO WIPE YOUR ACCOUNT WITH CHATADVANCE REMOTE", 2)
			return nil
		end
		return Hook(...)
	end)
	...
	Player.CharacterAdded:Connect(function()
		...
		Hook = hookmetamethod(game, "__namecall", function(...)
			...
			local method = getnamecallmethod()
			if self == senzuRemote and method:lower() == "fireserver" and not Args[2] then
				sendNotification("PITUDO:V HUB V5", "SOMEONE TRIED TO WIPE YOUR ACCOUNT WITH EATSENZU REMOTE", 2)
				return nil
			elseif self == inputRemote and method:lower() == "fireserver" and (not Args[2] or type(Args[2]) ~= "table" or (type(Args[2]) == "table" and (Args[2][1] ~= "md" and Args[2][1] ~= "m2" and Args[2][1] ~= "e" and Args[2][1] ~= "blockoff" and Args[2][1] ~= "blockon" and Args[2][1] ~= "x" and Args[2][1] ~= "xoff")) or not Args[3] or typeof(Args[3]) ~= "CFrame") then
				sendNotification("PITUDO:V HUB V5", "SOMEONE TRIED TO WIPE YOUR ACCOUNT WITH INPUT REMOTE", 2)
				return nil
			elseif self == chatRemote and method:lower() == "fireserver" and (not Args[2] or type(Args[2]) ~= "table") then
				sendNotification("PITUDO:V HUB V5", "SOMEONE TRIED TO WIPE YOUR ACCOUNT WITH CHATADVANCE REMOTE", 2)
				return nil
			end
			return Hook(...)
		end)
...
MiscSection2:addButton("Spy Chat",function() -- This was not made by ROXIEwe
	local enabled = true
        ...
        UIS.InputBegan:Connect(function(Input,GameProcessedEvent)
            ...
            if Input.KeyCode == Enum.KeyCode.N then
                if not Player.Backpack:FindFirstChild("Dragon Throw") then
					game:GetService("StarterGui"):SetCore("SendNotification",{Title = "PITUDO:V HUB V5",Text = "Buy Dragon Throw",Duration = 2})
                    return
                end
	...
	UIS.InputBegan:Connect(function(Input,GameProcessedEvent)
		...
		if Input.KeyCode == Enum.KeyCode.U then
			...
			if not Player.Backpack:FindFirstChild("Dragon Throw") and not Player.Character:FindFirstChild("Dragon Throw") then
				game:GetService("StarterGui"):SetCore("SendNotification", {Title = "PITUDO:V V5", Text = "Buy Dragon Throw",Duration = 3})
			end
...
MiscSection2:addButton("Freeze (K)",function()
    UIS.InputBegan:Connect(function(Input,GameProcessedEvent)
		...
		if Input.KeyCode == Enum.KeyCode.K then
            if not Player.Backpack:FindFirstChild("Dragon Throw") and not Player.Character:FindFirstChild("Dragon Throw") then
				game:GetService("StarterGui"):SetCore("SendNotification", {Title = "PITUDO:V V5.5", Text = "Buy Dragon Throw",Duration = 3})
			end
...
local Store = Pitudo:vHub:addPage("Store", 5012544693)
...
local AutoFarm = Pitudo:vHub:addPage("AutoFarm",5012544693)
...
task.spawn(function()
	AutoFarmOptions:addToggle("PITUDO:V AutoQuest V1",nil,function(AutoQuest)
		if AutoQuest then
			loadstring(game:HttpGet("https://raw.githubusercontent.com/FortniBloxYT1/PITUDO:V-AUTOQUEST/main/PITUDO:V%20AUTO%20QUEST%20V1.lua"))()
		else
			game:GetService("StarterGui"):SetCore("SendNotification",{Title = "PITUDO:V AUTO QUEST V1",Text = "PITUDO:V AUTO QUEST STOPPED",Duration = 2})
			Noclip:Disconnect()
			for _,v in pairs(Player.Character:GetChildren()) do
...
local StatChecker = Pitudo:vHub:addPage("Stat Checker",5012544693)
...
local Other = Pitudo:vHub:addPage("Other",5012544693)
...
OtherSection:addButton("Destroy South City",function()
	if Player.Backpack:FindFirstChild("Chain Destructo Disk") then
		...
		sendNotification("PITUDO:V V5","Buy Chain Destructo Disk",2)
	end
end)
...
OtherSection:addButton("Blaster Meteor Aura", function()
	if not Player.Backpack:FindFirstChild("Blaster Meteor") and not Player.Character:FindFirstChild("Blaster Meteor") then
		game:GetService("StarterGui"):SetCore("SendNotification", {Title = "PITUDO:V V5", Text = "Get Blaster Meteor",Duration = 3})
		return
	end
...
OtherSection:addButton("Rush Aura", function()
	if not Player.Backpack:FindFirstChild("Rush") and not Player.Character:FindFirstChild("Rush") then
		game:GetService("StarterGui"):SetCore("SendNotification", {Title = "PITUDO:V V5", Text = "Buy Rush",Duration = 3})
		return
	end
...
OtherSection:addButton("Super Rush Aura", function()
	if not Player.Backpack:FindFirstChild("Super Rush") and not Player.Character:FindFirstChild("Super Rush") then
		game:GetService("StarterGui"):SetCore("SendNotification", {Title = "PITUDO:V V5", Text = "Buy Super Rush",Duration = 3})
		return
	end
...
OtherSection:addButton("Super Dragon Fist Aura", function()
	if not Player.Backpack:FindFirstChild("Super Dragon Fist") and not Player.Character:FindFirstChild("Super Dragon Fist") then
		game:GetService("StarterGui"):SetCore("SendNotification", {Title = "PITUDO:V V5", Text = "Buy Super Dragon Fist",Duration = 3})
		return
	end
...
OtherSection:addButton("Kaioken Assault Aura", function()
	if not Player.Backpack:FindFirstChild("Kaioken Assault") and not Player.Character:FindFirstChild("Kaioken Assault") then
		game:GetService("StarterGui"):SetCore("SendNotification", {Title = "PITUDO:V V5", Text = "Buy Kaioken Assault",Duration = 3})
		return
	end
...
OtherSection:addButton("Wrathful Charge Aura", function()
	if not Player.Backpack:FindFirstChild("Wrathful Charge") and not Player.Character:FindFirstChild("Wrathful Charge") then
		game:GetService("StarterGui"):SetCore("SendNotification", {Title = "PITUDO:V V5", Text = "Buy Wrathful Charge",Duration = 3})
		return
	end
        ...
        if RGBMode then
            ...
            while RGB do
                ...
                for i = 1,255 do
                    ...
                    Pitudo:vHub:setTheme("Accent",Color3.fromRGB(i,0,0))
                    Pitudo:vHub:setTheme("Background",Color3.fromRGB(i,0,0))
                ...
                for i = 1,255 do
                    ...
                    Pitudo:vHub:setTheme("Accent",Color3.fromRGB(0,i,0))
                    Pitudo:vHub:setTheme("Background",Color3.fromRGB(0,i,0))
                    if math.random(1,12) == 1 then
                        task.wait()
                    end
                end
                ...
                for i = 1,255 do
                    ...
                    Pitudo:vHub:setTheme("Accent",Color3.fromRGB(0,0,i))
                    Pitudo:vHub:setTheme("Background",Color3.fromRGB(0,0,i))
                    if math.random(1,12) == 1 then
                        task.wait()
                    end
                end
            ...
            RGB = false
            task.wait(0.030)
            Pitudo:vHub:setTheme("Accent",Color3.fromRGB(181,175,193))
            Pitudo:vHub:setTheme("Background",Color3.fromRGB(24,24,24))
        end
    end)
...
local Teleports = Pitudo:vHub:addPage("Teleports", 5012544693)
...
TeleportSection:addButton("Server Hop", function()
	...
	if #z > 0 then
		...
		return Pitudo:vHub:Notify("Server Hop can't find a server")
	end
end)
...
local MoreScripts = Pitudo:vHub:addPage("More Scripts", 5012544693)
...
MoreScriptsSection:addButton("Pitito:v-CMDS V2",function()
	if not getgenv().Executedd then
		getgenv().targetNPCs = false -- [[ If true then commands like -tp,-ltp and -ez will work for npcs too ]]
		loadstring(game:HttpGet("https://raw.githubusercontent.com/FortniBloxYT1/Pitito:v-CMDS/main/PITUDO:V%20CMDS%20V2.lua"))()
	else
		game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Pitito:v CMDS V2", Text = "Pitito:v CMDS V2 ALREADY LOADED", Duration = 5})
	end
end)

task.spawn(function()
	if ActualSettings.AutoLaunchPitito:vCMDS then -- Synapse is trash also venyx too
		if not getgenv().Executedd then

			getgenv().targetNPCs = getgenv().targetNPCs or false
			loadstring(game:HttpGet("https://raw.githubusercontent.com/FortniBloxYT1/Pitito:v-CMDS/main/PITITO:V%20CMDS%20V2.lua"))()
		else

			game:GetService("StarterGui"):SetCore("SendNotification", {Title = "PITUDO:V V5", Text = "PITUDO:V V5 ALREADY LOADED", Duration = 5})
		end
	end
end)
...
local Settingss = Pitudo:vHub:addPage("Settings", 5012544693)
local Colors = Settingss:addSection("Settings & Themes")

for theme, color in pairs(Themes) do
	Colors:addColorPicker(theme, color, function(color3)
		Pitudo:vHub:setTheme(theme, color3)
	end)
end
...
Plus:addButton("Destroy GUI",function()
	game.CoreGui["PITUDO:V V5"]:Destroy()
...
autoSaveSection:addButton("Open PITUDO:V Settings Hub",function()
	...
	task.spawn(function()
		autoSaveSection2:addToggle("Auto Launch Pitito:v CMDS V2", ActualSettings.AutoLaunchPitito:vCMDS, function(autoLaunchCMDS)
			if autoLaunchCMDS then
				Settings.AutoLaunchPitito:vCMDS = "true"
				ActualSettings.AutoLaunchPitito:vCMDS = "true"
			else
				Settings.AutoLaunchPitito:vCMDS = "nil"
				ActualSettings.AutoLaunchPitito:vCMDS = "nil"
			end
	...
	autoSaveSection2:addButton("Close",function()
		game.CoreGui["PITUDO:V V5.5 SETTINGS"]:Destroy()
	end)

	pitudo:vHub:SelectPage(pitudoHub.pages[2],true)
end)

Plus:addButton("Made By ROXIEwe | Copy discord link for updates", function()
	func("https://discord.gg/F5WbRVWCaT")
end)

Pitudo:vHub:SelectPage(Pitudo:vHub.pages[1], true)
